...
const getAllPermutations = function (str) {
    let results = [];
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      let remainingCharsPerm = getAllPermutations(remainingChars);
      for (let j = 0; j < remainingCharsPerm.length; j++) {
        results.push(firstChar + remainingCharsPerm[j]);
      }
    }
    return results;
  }
